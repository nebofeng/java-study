...
public class Q8_MyAtoI {
    ...
    public  static  int myAtoi(String source){
          String s = source.trim();//去除首尾多余空格

          int zf=1;//定义符号
          boolean flag=false;// 是否计算了符号
          int result=0;
          for(int i = 0;i<s.length();i++){
            if(s.charAt(i)=='-'){
            ...
            }else if(s.charAt(i)>='0' && s.charAt(i)<='9'){//数字a
                //开始计算最终的数字， 如果不是
                ...
                if(!flag){
                    ...
                    //如果当前的result 大于 最大值除以10 ，或者 相等，且tmp >7 则正溢出
                    if(result>Integer.MAX_VALUE/10 || (result==Integer.MAX_VALUE/10 && tmp>7)) {
                        return  Integer.MAX_VALUE;
                    }else if(result<Integer.MIN_VALUE/10 || (result==Integer.MIN_VALUE/10 && tmp>8)){
                        return  Integer.MIN_VALUE;
                    }else{
                        result=result*10 + zf*tmp;
                    }
                }
            }else if(s.charAt(i)=='+'){
                if(i!=0){
